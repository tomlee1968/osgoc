...
##echo "Pausing to check repos to see whether base CentOS 6 repos are enabled"
##$SNIPPET('goc_install_pause')

## We want to get RPMs from the local mirrors, not from the main
## CentOS servers.
pushd /etc/yum.repos.d >&/dev/null
if ls CentOS-*.repo >&/dev/null; then
   for i in CentOS-*.repo; do
       mv $i $i.bak
   done
fi
popd >&/dev/null
...
## available to this postinstall script.  Note that it gets the base
## URL for the OSG repo from
## /usr/local/cobbler/repo_mirror/osg-<arch>-<rhelver>/config.repo,
## which doesn't get changed even if the repo is changed in Cobbler.
$yum_config_stanza
...
## Pause to make sure sssd can obtain 'goc' group, which
## goc_configure_svn (and many other things) requires
getent group goc
echo "Result: $?"
$SNIPPET('goc_install_pause')
...
yum install -y sssd-tools augeas augeas-libs facter munin-node puppet ruby-augeas ruby-shadow
yum install -y cilogon-ca-certs confsync-dyndns goc-ca-cert gociptables osg-ca-certs goc-crls osupdate gocloc munin_puppet
yum upgrade -y openssh
#unless $getVar('virtual', '')
yum install -y munin_gocipmi munin_ipc
#end unless
#if $getVar('stemcell', '')
yum install -y gocvminsta
#end if
## $SNIPPET('goc_install_pause')
...
## Note that Anaconda still doesn't install autofs, acpid, irqbalance,
## rpcbind, or rpcidmapd.
...
## $SNIPPET('goc_cleanup') causes the erasure of
## /etc/yum.repos.d/cobbler-config.repo, which means the local
## repositories aren't available anymore, and that's going to be a
## problem later.  Install the goc-internal-repo and osg-release RPMs.
